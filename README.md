...
#### 대시보드 (`/api/dashboard`)
- `GET /admin` – 관리자 종합 통계 (회원수/투자/지급/수당/출금 현황)
...
| 파일 | 기능 |
|------|------|
| `dashboard.html` | 종합 KPI · 직급수당 출금대기 목록(즉시 표시) · 주간 지급 예정 · 활동 로그 |
| `commission.html` | 직급수당 전체 목록 · 계좌번호 · 지급전/후잔고 · 개별/전체 출금완료 처리 |
| `withdrawal.html` | 출금 요청 관리 · 승인/거절 |
| `deposit.html` | 투자금 입금 등록 |
